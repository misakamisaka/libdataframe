...
void LeafExpression::Resolve(std::shared_ptr<Schema> schema) {
  nullable_ = true;
  resolved_ = true;
}
...
std::shared_ptr<DataField> PairExpression::Eval(std::shared_ptr<Row> row) {
...
std::shared_ptr<DataField> ArrayExpression::Eval(std::shared_ptr<Row> row) {
